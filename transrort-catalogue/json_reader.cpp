...
JsonReader::JsonReader(std::istream& input, transport::Catalogue& catalogue) 
    : input_(json::Load(input)), catalogue_(catalogue) {}
...
void JsonReader::FillCatalogue() {
    ...
    for (auto& request_bus : arr) {
        ...
        if (type == "Bus"s) {
            ...
            catalogue_.AddRoute(bus_number, stops, circular_route);
        }
    }
}
    ...
    for (auto& request_stops : arr) { 
        ...
        if (type == "Stop"s) { 
            ...
            for (auto& [to_name, dist] : stop_distances) { 
                auto from = catalogue_.FindStop(stop_name); 
                auto to = catalogue_.FindStop(to_name); 
...
std::tuple<std::string_view, std::vector<const transport::Stop*>, bool> 
JsonReader::FillRoute(const json::Dict& request_map) const {
    ...
    for (auto& stop : request_map.at("stops"s).AsArray()) { 
        stops.push_back(catalogue_.FindStop(stop.AsString())); 
    } 
...
renderer::MapRenderer JsonReader::FillRenderSettings(const json::Node& settings) const { 
    ...
    if (request_map.at("underlayer_color"s).IsString()) { 
        render_settings.underlayer_color = request_map.at("underlayer_color"s).AsString(); 
    ...
    for (const auto& color_element : color_palette) { 
        if (color_element.IsString()) { 
            ...
            if (color_type.size() == 3) { 
                ...
                throw std::logic_error("wrong color_palette type"s); 
            } 
        } else { 
            throw std::logic_error("wrong color_palette"s); 
        } 
    } 
...
const json::Node JsonReader::PrintRouting(const json::Dict& request_map, RequestHandler& rh) const {
    ...
    if (!routing) {
        ...
        json::Array items;
        double total_time = routing->total_time.count();
        items.reserve(routing->items.size());
        
        for (const auto& item : routing->items) {
            if (std::holds_alternative<transport::RouteInternalInfo::WaitItem>(item)) {
                const auto& wait_item = std::get<transport::RouteInternalInfo::WaitItem>(item);
                items.emplace_back(json::Builder{}
                    .StartDict()
                        .Key("stop_name"s).Value(wait_item.stop_ptr->name)
                        .Key("time"s).Value(wait_item.time.count())
                        .Key("type"s).Value("Wait"s)
                    .EndDict()
                .Build());
            } else {
                const auto& bus_item = std::get<transport::RouteInternalInfo::BusItem>(item);
                items.emplace_back(json::Builder{}
                    .StartDict()
                        .Key("bus"s).Value(bus_item.bus_ptr->number)
